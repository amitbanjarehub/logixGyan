...
const Logout_attendance = () => {
  const aCheckOutMutation = useMutation(attendanceService.checkOut, {
    onError(error) {
      console.log(error);
    },
    onSuccess: (data) => {
  ...
  const onAdd = (lat, long, checkOutselfie, meterEnd, meterEndNumber) => {
    aCheckOutMutation.mutate({
      lat,
      long,
      checkOutselfie,
      meterEnd,
            ...
            <Formik
              ...
              onSubmit={async (data, { setSubmitting, resetForm }) => {
                ...
                await onAdd(
                  (data.lat = latitude),
                  (data.long = longitude),
                  (data.checkOutselfie = source),
                  (data.meterEnd = source1),
                  data.meterEndNumber
                );
                setSubmitting(false);
              }}
                  ...
                  >
                    ...
                    <Div sx={{ mx: 1 }}>
                      <LoadingButton
                        ...
                        size="large"
                        sx={{ mb: 3 }}
                        loading={isSubmitting || aCheckOutMutation.isLoading}
