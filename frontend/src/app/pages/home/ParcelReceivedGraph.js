...
import ParcelReceivedChart from "./ParcelReceivedChart";

const ParcelReceivedGraph = ({ branchId }) => {
  let total = 0;
  ...
  const liwParcel = ParcelReceived?.data?.data ?? [];
  ...
  const ReceivedParcelData = Object.values(groupByUserId).map((item) => {
    ...
    for (let i = 0; i < item.length; i++) {
      total += Number(item[i]?.noOfParcel ?? 0);
    ...
    for (let i = 0; i < AcutalDays; i++) {
      ...
      checkData.push({ date: bdate, Parcel_Received: numberAlloted ? parcelAllotData : 0 })
  ...
  return (
    ...
    >
      <Div sx={{ mt: -2.5 }} style={{ marginTop: "20px" }}>
        <ParcelReceivedChart chartData={ReceivedGraph} />
      </Div>
    </JumboCardQuick>
  );
};
...
export default ParcelReceivedGraph;
