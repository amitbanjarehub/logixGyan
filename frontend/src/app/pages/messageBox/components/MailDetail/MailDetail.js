...
const MailDetail = ({ mailID }) => {
  ...
  const mailMoveToTrash = React.useCallback((mailID) => {
    ...
    navigate(`/mails/messages/${category}`);
  }, []);
  ...
  const mailMoveToArchived = React.useCallback(
    (mailID) => {
  ...
  const mailMoveToSpam = React.useCallback(
    (mailID) => {
  ...
  const { data, isLoading, isError, error } = useQuery(
    ["mailDetail", mailID],
    () => mailServices.getMailDetail(mailID)
  );
  if (isLoading) {
    return (
      <Div
  ...
  const handleNavigateItem = () => {
    if (category && id) return navigate(`/mails/messages/${category}/${id}`);
    return navigate(`/mails/messages/${category}`);
  };
  return (
    <Card>
      <CardHeader
        ...
        >
          ...
          >
            <Div sx={{ mb: 1 }}>
              ...
              >
                <Avatar
                  src={data?.mailDetail?.from.profile_pic}
