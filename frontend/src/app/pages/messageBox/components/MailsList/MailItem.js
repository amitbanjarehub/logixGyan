...
const MailItem = ({ mailItem }) => {
  ...
  const handleConversationClick = () => {
    ...
    navigate(`/mails/messages/${mailItem.id}`);
  };
  const deleteMailMutation = useMutation(mailServices.deleteMail);

  const mailMoveToTrash = React.useCallback(
    (mailID) => {
  ...
  const handleRemoveFavoriteMail = React.useCallback(
    (mailID) => {
        ...
        <React.Fragment>
          ...
          >
            <ListItemIcon sx={{ display: { xs: "none", md: "block" } }}>
              <IconButton
                onClick={() => handleRemoveFavoriteMail(mailItem?.id)}
              >
                {favoriteMail ? (
              ...
              >
                <Span sx={{ height: 36, overflow: "hidden" }}>
                  ...
                  <Tooltip title={"Archive"}>
                    <IconButton
                      onClick={() => mailMoveToArchived(mailItem?.id)}
                    >
