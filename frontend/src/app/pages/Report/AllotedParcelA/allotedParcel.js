...
const ParcelAllotedReport = () => {
  ...
  const columns = [
    ...
    {
      name: "totalParcelAlloted",
      label: "Total No of Alloted Parcel",
      options: { filterOptions: { fullWidth: true } },
    },
  ];
  ...
  const AttendaceData = Object.values(groupByUserId).map((item) => {
    ...
    for (let i = 0; i < item.length; i++) {
      attendance += Number(item[i]?.noOfparcelAlloted ?? 0);
    }

    const data = {
      Name: userName,
      MobileNo: userMobile,
      totalParcelAlloted: attendance,
  ...
  console.log("dashboard", AttendaceData);
  let totalallotedParcelD = 0;
  for (let m = 0; m < AttendaceData.length; m++) {
    totalallotedParcelD = totalallotedParcelD + AttendaceData[m].totalParcelAlloted;
  }
  console.log("totalallotedParcelD:", totalallotedParcelD);
...
export default ParcelAllotedReport;
