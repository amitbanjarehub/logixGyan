...
const RejectedParcel = () => {
  ...
  const columns = [
    ...
    {
      name: "totalRejectedParcel",
      label: "Total No of Rejected Parcel",
      options: { filterOptions: { fullWidth: true } },
    },
  ];
  ...
  console.log("liwParcelData", liwParcelData);
  ...
  const [totalParcelNo, setTotalParcelNo] = useState();
  console.log("totalParcelNo", groupByUserId);
  ...
  const AttendaceData = Object.values(groupByUserId).map((item) => {
    ...
    for (let i = 0; i < item.length; i++) {
      attendance += Number(item[i]?.rejectParcel ?? 0);
    }
    ...
    const data = {
      Name: userName,
      MobileNo: userMobile,
      totalRejectedParcel: attendance,
    ...
    for (let i = 0; i < AcutalDays; i++) {
      ...
      const numberAlloted = item.find((e) => e.date === date);
      const parcelAllotData = numberAlloted?.rejectParcel;
