...
const ReportRejectedParcel = () => {
  ...
  const columns = [
    ...
    {
      name: "totalParcelRejected",
      label: "Total No of Parcel Rejected",
      options: { filterOptions: { fullWidth: true } },
    },
  ];
  ...
  const AttendaceData = Object.values(groupByUserId).map((item) => {
    ...
    const data = {
      Name: userName,
      MobileNo: userMobile,
      totalParcelRejected: attendance,
    };
    for (let i = 0; i < AcutalDays; i++) {
      const date = moment(FirstDay)
        .date(i + 1)
          ...
          <Div sx={{ display: "flex", flexDirection: "column", flex: "1" }}>
            <CardContent>
              <Typography component={"h2"} variant="h1" mb={3}>
                Total Number of Rejected Parcel
              </Typography>
              <Box
                component="form"
                sx={{
        ...
        <Div>
          <CacheProvider value={muiCache}>
            {wComp.isLoading ? (
              ...
              <MUIDataTable
                title={"Total Number of Rejected Parcel"}
                data={AttendaceData}
                columns={columns}
                options={options}
              />
...
export default ReportRejectedParcel;
