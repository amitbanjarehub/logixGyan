...
const ExpenseReport = () => {
  ...
  console.log("authUser8989:--------->>>>", authUser);
  ...
  const ExpenseData = useQuery(["ExpenseData", { branchId, FirstDay, LastDay }],
    async () => branchServices.expenseslist(branchId, FirstDay, LastDay)
  );

  const liwexpenseData = ExpenseData?.data?.data ?? [];

  console.log("liwexpenseData11:------->>>>", liwexpenseData);

  const groupByUserId = liwexpenseData.reduce(function (a, e) {
    let estKey = e["userId"];
  ...
  const AttendaceData = Object.values(groupByUserId).map((item) => {
    ...
    console.log("item3232:", item);
    let attendance = 0;

    for (let i = 0; i < item.length; i++) {
      attendance += Number(item[i]?.ammount ?? 0);
    }
    ...
    for (let i = 0; i < AcutalDays; i++) {
      ...
      const numberExpenseAmount = item.filter((e) => e.date === date);
      let expenseAmountData = 0;
      if (numberExpenseAmount?.length) {
        for (let j = 0; j < numberExpenseAmount?.length; j++) {
          expenseAmountData += numberExpenseAmount[j]?.ammount;
        }
      }

      data[bdate] = numberExpenseAmount ? expenseAmountData : 0;

    }
    console.log("item7676", item);
    console.log("data5656", data);
